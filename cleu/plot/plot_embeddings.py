...
def plot_confusion_similarity(list_embedding_src,list_embedding_tgt,distance_function='cosine',csls_k=5):
    ...
    words_id = list(map(lambda embedding: embedding.word , list_embedding_src))
    words_en = list(map(lambda embedding: embedding.word , list_embedding_tgt))
    ...
    ax = sns.heatmap(similarity_matrix ,annot=True,xticklabels=words_en, yticklabels=words_id)
    ax.set_title(f'Cross-domain Similarity Local Scaling')
